...
function wrapText(value, maxChars, maxLines) {
  ...
  if (lines.length === maxLines && words.join(" ").length > lines.join(" ").length) {
    lines[maxLines - 1] = `${lines[maxLines - 1].slice(0, Math.max(0, maxChars - 3)).trimEnd()}...`;
  }
...
function renderScreen(rawState) {
  ...
  const messageLines = wrapText(state.message, 35, 2);
...
<svg xmlns="http://www.w3.org/2000/svg" width="330" height="330" viewBox="0 0 330 330" role="img" aria-labelledby="title desc">
  ...
  <text x="38" y="265" fill="#111827" font-family="Consolas, monospace" font-size="11" font-weight="700">${escapeXml(messageLines[0])}</text>
  <text x="38" y="279" fill="#111827" font-family="Consolas, monospace" font-size="11" font-weight="700">${escapeXml(messageLines[1] || "")}</text>
  <text x="38" y="294" fill="#374151" font-family="Consolas, monospace" font-size="10">${status}</text>
